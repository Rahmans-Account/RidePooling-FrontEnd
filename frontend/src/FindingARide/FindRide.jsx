      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-slate-500 max-w-xl mx-auto font-medium">
            Search for verified rides heading your way. Simple, secure, and
            sustainable travel.
          </p>
        </div>
                ...
                <AutoCompleteLocation
                  ...
                  onSelect={(loc) =>
                    setCurrentLocation({
                      name: loc.name,
                      lat:
                        loc.lat ?? loc.latitude ?? loc.geometry?.location?.lat,
                      lng:
                        loc.lng ??
                        loc.lon ??
                        loc.longitude ??
            ...
            <div className="lg:col-span-2 relative px-4 lg:border-l lg:border-slate-100">
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  value={minSeats}
                  onChange={(e) =>
                    setMinSeats(Math.max(1, Number(e.target.value)))
                  }
                />
              </div>
            </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[1, 2, 3].map((n) => (
                <div
                  key={n}
                  className="h-64 bg-white border border-slate-100 rounded-[2.5rem] animate-pulse"
                />
              ))}
            </div>
          ) : rides.length === 0 ? (
            <div className="bg-white rounded-[3rem] py-20 px-6 text-center border border-dashed border-slate-200">
              ...
              <p className="text-slate-500 mt-2 max-w-xs mx-auto font-medium">
                Try adjusting your location or selecting a different date to
                find available commutes.
              </p>
            </div>
          ) : (
